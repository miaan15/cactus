...
export struct ArchetypeTable {
    ...
    ArchetypeTable(size_t row_count) : row_count(row_count) {}
    ...
    [[nodiscard]] auto create_row() -> size_t {
        if (row_count >= capacity) {
            size_t new_capacity = capacity == 0 ? 4 : capacity + (capacity >> 1);

            void *new_table_ptr = realloc(table_ptr, new_capacity * row_size);
            assert(new_table_ptr);
            table_ptr = new_table_ptr;

            Entity *new_entity_list =
        ...
        memset(get_row_ptr(row_count), 0, row_size);
