...
// // TOKEN
// // ======================================================================================
// enum TokenType : int { IDENTIFIER = 256, NUMBER, STRING };
// struct Token {
//     TokenType type;
//     size_t len;
//
...
// // TODO handle the file reading and stuff should be by other thing
// // SPINES CONTEXT
// // ======================================================================================
// struct SpinesContext {
...
//             [](SpinesParseError::Type t) {
//                 return SpinesParseError{.type = t};
//             });
//         if (!spines_source_exp.has_value())
//             return std::unexpected{spines_source_exp.error()};
...
//             IdentifierData old_identifier_data =
//                 identifier_data_list.get(identifier_stack.top());
...
//                 identifier_data_list.get_ptr(identifier_stack.top());
...
//                 ++next_identifier_id;
//
//                 // NOTE: cheat flag by set it = 2 then decrease 2 times
//                 // (because I dont want to somehow exit this case without decrease this flag value)
...
//                 int i_val;
//                 auto res_i =
//                     std::from_chars(strv.data(), strv.data() + strv.size(), i_val);
//
...
//                     float f_val;
//                     auto res_f =
//                         std::from_chars(strv.data(), strv.data() + strv.size(), f_val);
//
//                     if (res_f.ptr == strv.data() + strv.size()) {
//                         data.append(f_val);
//                     } else {
//                         return std::unexpected{SpinesParseError{
//                             .type = parsing_error_type_t::INVALID_VALUE,
//                             .index = token.index,
//                             .column = token.column,
//                             .line = token.line}};
//                     }
...
//             default: {
//                 return std::unexpected{SpinesParseError{
//                     .type = parsing_error_type_t::INVALID_SYNTAX,
//                     .index = token.index,
//                     .column = token.column,
//                     .line = token.line}};
//
...
//                 [](unsigned char c) { return c == '\r' || c == '\v' || c == '\f'; }),
//                 content.end());
//
...
//         };
//
//         auto get_identifier_len =
//             [&](const std::string_view &trimmed_content_view) noexcept -> size_t {
//                 size_t len = 0;
...
//             };
//
//         auto might_be_value = [&](char c) noexcept -> bool {
...
//                     return std::unexpected{SpinesParseError{
//                         .type = parsing_error_type_t::INVALID_IDENTIFIER_NAME,
...
//                     if (end_string_i != 0)
//                         append_token(TokenType::STRING, end_string_i);
//
...
//             if (error.has_value()) return Accessor{error, ctx_ref, chain_depth};
//
//             _assert(ctx_ref != nullptr, "Context reference is null.");
//             _assert(cur_identifier_index < ctx_ref->identifier_data_list.len, "cur_identifier_index is out of bounds.");
//
//             IdentifierData cur_identifier_data =
//                 ctx_ref->identifier_data_list.get(cur_identifier_index);
//
//             if (data_offset.has_value())
//                 return Accessor{Error::INVALID_OPERATION, ctx_ref, chain_depth + 1};
//
//             size_t search_start = cur_identifier_index + 1;
//             size_t search_end = cur_identifier_index + cur_identifier_data.parent_len;
//             _assert(search_end <= ctx_ref->identifier_data_list.len, "Search range exceeds identifier_data_list bounds.");
//
//             for (size_t i = search_start; i < search_end;) {
//                 auto data = ctx_ref->identifier_data_list.get(i);
//
//                 _assert(data.name_offset_root + data.name_len <= ctx_ref->spines_source.size(),
//                         "String view out of bounds of spines_source.");
//
//                 std::string_view cur_name{
//                     ctx_ref->spines_source.data() + data.name_offset_root, data.name_len};
//                 if (cur_name == name)
//                     return Accessor{error, ctx_ref, chain_depth + 1, i, {}};
//
//                 i += data.parent_len;
//             }
//
//             return Accessor{Error::NAME_NOT_FOUND, ctx_ref, chain_depth + 1};
//         }
//
...
//                 return Accessor{Error::INDEX_OUT_OF_BOUND, ctx_ref, chain_depth + 1};
//             }
//
//             return Accessor{error, ctx_ref, chain_depth + 1, cur_identifier_index, index};
//         }
//
//         template <typename T> [[nodiscard]]
//         std::expected<T, std::pair<Error, size_t>> as() const noexcept {
//             auto data_index_opt = handle_pre_check_on_get_data_index();
//             if (!data_index_opt.has_value())
//                 return std::unexpected{data_index_opt.error()};
//
//             auto val = ctx_ref->data.get(data_index_opt.value());
//
...
//                 _assert(str_idx < ctx_ref->string_data.len, "String index out of bounds of string_data container.");
//
//                 const char* string_raw = ctx_ref->string_data.get_ptr(str_idx);
//                 _assert(string_raw, "String data should not be null");
//
//                 if (string_raw - ctx_ref->string_data.data >= ctx_ref->string_data.len)
...
//                     ++string_size;
//
//                     if (string_raw + string_size - ctx_ref->string_data.data > ctx_ref->string_data.len)
//                         return std::unexpected{std::make_pair(Error::STRING_OVERFLOW, chain_depth + 1)};
//                 }
//
...
//
//         [[nodiscard]]
//         std::expected<int *, std::pair<Error, size_t>> as_int_ptr() noexcept {
//             auto data_index_opt = handle_pre_check_on_get_data_index();
//             if (!data_index_opt.has_value()) return std::unexpected{data_index_opt.error()};
//
//             auto val = ctx_ref->data.get_ptr(data_index_opt.value());
//
...
//         }
//         [[nodiscard]]
//         std::expected<float *, std::pair<Error, size_t>> as_float_ptr() noexcept {
//             auto data_index_opt = handle_pre_check_on_get_data_index();
//             if (!data_index_opt.has_value()) return std::unexpected{data_index_opt.error()};
//
//             auto val = ctx_ref->data.get_ptr(data_index_opt.value());
...
//         }
//         [[nodiscard]]
//         std::expected<std::span<char>, std::pair<Error, size_t>> as_string_span() noexcept {
//             auto data_index_opt = handle_pre_check_on_get_data_index();
//             if (!data_index_opt.has_value()) return std::unexpected{data_index_opt.error()};
//
//             auto val = ctx_ref->data.get(data_index_opt.value());
...
//             _assert(str_idx < ctx_ref->string_data.len, "String index out of bounds of string_data container.");
//
//             char* string_raw = ctx_ref->string_data.get_ptr(str_idx);
//             _assert(string_raw, "String data should not be null");
//
//             if (string_raw - ctx_ref->string_data.data >= ctx_ref->string_data.len)
//                 return std::unexpected{std::make_pair(Error::STRING_OVERFLOW, chain_depth + 1)};
...
//
//         [[nodiscard]]
//         std::optional<std::pair<Error, size_t>> get_error() const noexcept {
...
//             _assert(cur_identifier_index < ctx_ref->identifier_data_list.len, "cur_identifier_index is out of bounds.");
//
//             IdentifierData cur_identifier_data = ctx_ref->identifier_data_list.get(cur_identifier_index);
//
...
//             _assert(data_index < ctx_ref->data.len, "data_index is out of bounds.");
//
//             return data_index;
//         }
//     };
...
//             auto data = identifier_data_list.get(i);
